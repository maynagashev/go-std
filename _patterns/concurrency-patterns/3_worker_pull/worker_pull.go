/*
Паттерн Worker Pool, или Пул рабочих процессов (также известный как Пул потоков) — это шаблон многопоточности,
в котором создаётся пул из рабочих процессов для одновременной обработки задач из очереди.

Главные компоненты паттерна: система очередей для доставки задач воркерам (работникам) и логика получения задач.
Обычно система очередей представляет собой канал, а логика получения задач заключается в том,
...
/*
рабочий 3 запущен задача 1
рабочий 1 запущен задача 2
рабочий 2 запущен задача 3
рабочий 1 закончил задача 2
рабочий 1 запущен задача 4
рабочий 3 закончил задача 1
рабочий 2 закончил задача 3
рабочий 2 запущен задача 5
рабочий 1 закончил задача 4
